...
def inicio():
    ...
    existentes = [os.path.splitext(filename)[0]
...
def input_image():
    ...
    while int(contProd) != int(totalProd):
        ...
        produto = str(
            input("Digite o {prod} produto: ".format(prod=produtos[contProd])))
...
def draw_image(produto):
    ...
    if totalProd == 1:
        x = [100]
        y = [300]
        pdf.drawImage('./imgs/'+produto+'.jpg',
                      x[contProd], y[contProd], width=388, height=272)
